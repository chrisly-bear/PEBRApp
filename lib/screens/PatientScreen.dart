...
class _PatientScreenState extends State<PatientScreen> {
  ...
  void initState() {
    ...
    getLatestViralLoadFetchFromSharedPrefs(_patient.artNumber)
        .then((DateTime fetchDate) {
      setState(() {
        lastVLFetchDate =
            fetchDate == null ? 'never' : formatDateAndTime(fetchDate);
      });
    });
    _appStateStream = PatientBloc.instance.appState.listen((streamEvent) {
      if (streamEvent is AppStatePatientData &&
        ...
        print(
            '*** PatientScreen received AppStatePatientData: ${streamEvent.patient.artNumber} ***');
  ...
  Widget build(BuildContext context) {
    ...
    DateTime lastAssessmentDate =
        _patient.latestPreferenceAssessment?.createdDate;
    DateTime nextAssessmentDate =
        calculateNextAssessment(lastAssessmentDate, isSuppressed(_patient)) ??
            _patient.enrollmentDate;
    _nextAssessmentText = formatDate(nextAssessmentDate);

    DateTime nextRefillDate =
        _patient.latestDoneARTRefill?.nextRefillDate ?? _patient.enrollmentDate;
    _nextRefillText = formatDate(nextRefillDate);

    DateTime nextEndpointSurveyDate = calculateNextQuestionnaire(
        _patient.enrollmentDate, _patient.requiredActions);
    ...
    final Widget content = Column(
      children: <Widget>[
        ...
        !_patient.isActivated
            ? _makeButton('Edit Characteristics', flat: true)
        ...
        !_patient.isActivated
            ? _makeButton('fetch from database', flat: true)
            : _makeButton(
                'fetch from database',
                onPressed: _isFetchingViralLoads
                    ? null
                    : () {
                        _fetchFromDatabasePressed(_context, _patient);
                      },
                    ...
                    ? SizedBox(
                        ...
                        child: CircularProgressIndicator(
                            valueColor: AlwaysStoppedAnimation<Color>(
                                SPINNER_PATIENT_SCREEN_FETCH_VIRAL_LOADS)))
                    : null,
                flat: true,
              ),
        Padding(
          padding: EdgeInsets.symmetric(horizontal: 20.0),
          child: Text('Last fetch: ${lastVLFetchDate ?? 'never'}',
              textAlign: TextAlign.center),
        ),
        !_patient.isActivated
            ? _makeButton('add manual entry', flat: true)
            : _makeButton(
                'add manual entry',
                onPressed: _isFetchingViralLoads
                    ? null
                    : () {
                        _addManualEntryPressed(_context, _patient);
                      },
                flat: true,
              ),
        Padding(
          padding: EdgeInsets.symmetric(horizontal: 20.0),
          child: Text(
              'Use this option to correct a wrong entry from the database.',
              textAlign: TextAlign.center),
        ),
        SizedBox(height: _spacingBetweenCards),
      ...
      children: <Widget>[
        _userData?.healthCenter?.studyArm == 2
            ...
            : _buildNextActionRow(
                ...
                explanation:
                    'Preference assessments are due every month for unsuppressed participants and every 3 months for suppressed participants.',
                button: !_patient.isActivated
                    ? _makeButton('Start Assessment')
                    : _makeButton('Start Assessment', onPressed: () {
                        _startAssessmentPressed(_context, _patient);
                      }),
              ),
        SizedBox(height: _spacingBetweenCards),
        _buildNextActionRow(
          ...
          button: !_patient.isActivated
              ...
              : _makeButton('Manage Refill', onPressed: () {
                  _manageRefillPressed(_context, _patient, _nextRefillText);
                }),
        ),
        SizedBox(height: _spacingBetweenCards),
        _buildNextActionRow(
  ...
  _buildViralLoadHistoryCard() {
    ...
    bool _hasAnyDiscrepancies() =>
        _patient.viralLoads.any((ViralLoad vl) => vl.discrepancy ?? false);
    bool _vlWarningRequiredActionShowing() =>
        _patient.requiredActions.any((RequiredAction a) =>
            a.type == RequiredActionType.VIRAL_LOAD_DISCREPANCY_WARNING);
    bool _shouldDisplayWarningColumn() =>
        _hasAnyDiscrepancies() && _vlWarningRequiredActionShowing();
    ...
    Widget _buildViralLoadRow(vl, {bool bold: false}) {
      ...
      Widget _viralLoadIcon(ViralLoad vl) {
        ...
        if (vl.failed) {
          return _getPaddedIcon('assets/icons/viralload_failed.png',
              width: vlIconSize, height: vlIconSize);
        } else if (vl.isSuppressed) {
          return _getPaddedIcon('assets/icons/viralload_suppressed.png',
              width: vlIconSize, height: vlIconSize);
        } else if (!vl.isSuppressed) {
          return _getPaddedIcon('assets/icons/viralload_unsuppressed.png',
              width: vlIconSize, height: vlIconSize);
        }
        return Text('—');
      }
      ...
      return Padding(
        ...
        child: Row(
          children: <Widget>[
            Expanded(
                child: Text(formatDateConsistent(vl.createdDate),
                    style: TextStyle(
                        fontWeight:
...
//              viralLoadBadge,
            ...
            SizedBox(
              width: _sourceColumnWidth,
              child: Text(
                  vl.source == ViralLoadSource.MANUAL_INPUT()
                      ? 'manual'
                      : 'database',
                  style: TextStyle(
            ...
            _shouldDisplayWarningColumn()
                ? SizedBox(
                    ...
                    child:
                        (vl.discrepancy ?? false) ? Icon(Icons.warning) : null,
                  )
                : SizedBox(),
          ],
        ),
    ...
    if (_patient.viralLoads.length == 0) {
      ...
      _patient.viralLoads.asMap().forEach((int i, ViralLoad vl) {
        vlsMarkedAsBold
            .add({'vl': vl, 'bold': numOfVLs > 1 && i == numOfVLs - 1});
      });
    ...
    return Column(
      ...
      children: [
        ...
        _buildExplanation(
            'All available viral loads for this participant up to one '
          ...
          child: SingleChildScrollView(
            ...
            child: ConstrainedBox(
              constraints: BoxConstraints(
                minWidth: 550.0,
                maxWidth:
                    max(550.0, MediaQuery.of(context).size.width - 2 * 15.0),
              ),
              child: Padding(
  ...
  _buildPreferencesCard() {
    Widget _buildSupportOptions() {
      ...
      if (sps.areAllDeselected) {
        return _buildRowWithWidget(
          'Support',
          Row(
            children: [
              _getPaddedIcon('assets/icons/no_support.png',
      ...
      if (sps.NURSE_CLINIC_selected) {
        supportOptions.add(_buildSupportOption(
          ...
          doneText:
              'done ${formatDateAndTimeTodayYesterday(_pa.NURSE_CLINIC_done_date)}',
          ...
          icon: _getPaddedIcon('assets/icons/nurse_clinic.png',
              width: iconWidth,
              height: iconHeight,
              color: _pa.NURSE_CLINIC_done ? ICON_INACTIVE : ICON_ACTIVE),
        ));
      }
      if (sps.SATURDAY_CLINIC_CLUB_selected &&
          _pa.saturdayClinicClubAvailable) {
        supportOptions.add(_buildSupportOption(
          SupportOption.SATURDAY_CLINIC_CLUB().description,
          checkboxState: _pa.SATURDAY_CLINIC_CLUB_done,
          doneText:
              'done ${formatDateAndTimeTodayYesterday(_pa.SATURDAY_CLINIC_CLUB_done_date)}',
          onChanged: (bool newState) {
            setState(() {
          ...
          icon: _getPaddedIcon('assets/icons/saturday_clinic_club.png',
              width: iconWidth,
              height: iconHeight,
              color:
                  _pa.SATURDAY_CLINIC_CLUB_done ? ICON_INACTIVE : ICON_ACTIVE),
        ));
      }
      if (sps.COMMUNITY_YOUTH_CLUB_selected &&
          _pa.communityYouthClubAvailable) {
        supportOptions.add(_buildSupportOption(
          SupportOption.COMMUNITY_YOUTH_CLUB().description,
          checkboxState: _pa.COMMUNITY_YOUTH_CLUB_done,
          doneText:
              'done ${formatDateAndTimeTodayYesterday(_pa.COMMUNITY_YOUTH_CLUB_done_date)}',
          onChanged: (bool newState) {
            setState(() {
          ...
          icon: _getPaddedIcon('assets/icons/youth_club.png',
              width: iconWidth,
              height: iconHeight,
              color:
                  _pa.COMMUNITY_YOUTH_CLUB_done ? ICON_INACTIVE : ICON_ACTIVE),
        ));
      }
      if (sps.PHONE_CALL_PE_selected) {
        supportOptions.add(_buildSupportOption(
          ...
          doneText:
              'done ${formatDateAndTimeTodayYesterday(_pa.PHONE_CALL_PE_done_date)}',
          onChanged: (bool newState) {
            setState(() {
          ...
          icon: _getPaddedIcon('assets/icons/phonecall_pe.png',
              width: iconWidth,
              height: iconHeight,
              color: _pa.PHONE_CALL_PE_done ? ICON_INACTIVE : ICON_ACTIVE),
        ));
      }
      if (sps.HOME_VISIT_PE_selected && _pa.homeVisitPEPossible) {
        supportOptions.add(_buildSupportOption(
          ...
          doneText:
              'done ${formatDateAndTimeTodayYesterday(_pa.HOME_VISIT_PE_done_date)}',
          onChanged: (bool newState) {
            setState(() {
          ...
          icon: _getPaddedIcon('assets/icons/homevisit_pe.png',
              width: iconWidth,
              height: iconHeight,
              color: _pa.HOME_VISIT_PE_done ? ICON_INACTIVE : ICON_ACTIVE),
        ));
      }
      if (sps.SCHOOL_VISIT_PE_selected && _pa.schoolVisitPEPossible) {
        String schoolNameAndVillage =
            _patient.latestPreferenceAssessment?.school;
        schoolNameAndVillage =
            schoolNameAndVillage == null ? '' : '\n($schoolNameAndVillage)';
        supportOptions.add(_buildSupportOption(
          SupportOption.SCHOOL_VISIT_PE().description + schoolNameAndVillage,
          checkboxState: _pa.SCHOOL_VISIT_PE_done,
          doneText:
              'done ${formatDateAndTimeTodayYesterday(_pa.SCHOOL_VISIT_PE_done_date)}',
          onChanged: (bool newState) {
            setState(() {
          ...
          icon: _getPaddedIcon('assets/icons/schooltalk_pe.png',
              width: iconWidth,
              height: iconHeight,
              color: _pa.SCHOOL_VISIT_PE_done ? ICON_INACTIVE : ICON_ACTIVE),
        ));
      }
      if (sps.PITSO_VISIT_PE_selected && _pa.pitsoPEPossible) {
        supportOptions.add(_buildSupportOption(
          ...
          doneText:
              'done ${formatDateAndTimeTodayYesterday(_pa.PITSO_VISIT_PE_done_date)}',
          onChanged: (bool newState) {
            setState(() {
          ...
          icon: _getPaddedIcon('assets/icons/pitso.png',
              width: iconWidth,
              height: iconHeight,
              color: _pa.PITSO_VISIT_PE_done ? ICON_INACTIVE : ICON_ACTIVE),
        ));
      }
      if (sps.CONDOM_DEMO_selected) {
        supportOptions.add(_buildSupportOption(
          ...
          doneText:
              'done ${formatDateAndTimeTodayYesterday(_pa.CONDOM_DEMO_done_date)}',
          onChanged: (bool newState) {
            setState(() {
      ...
      if (sps.CONTRACEPTIVES_INFO_selected) {
        String contraceptivesPerson =
            _patient.latestPreferenceAssessment?.moreInfoContraceptives;
        contraceptivesPerson =
            contraceptivesPerson == null ? '' : '\n($contraceptivesPerson)';
        supportOptions.add(_buildSupportOption(
          SupportOption.CONTRACEPTIVES_INFO().description +
              contraceptivesPerson,
          checkboxState: _pa.CONTRACEPTIVES_INFO_done,
          doneText:
              'done ${formatDateAndTimeTodayYesterday(_pa.CONTRACEPTIVES_INFO_done_date)}',
          onChanged: (bool newState) {
            setState(() {
      ...
      if (sps.VMMC_INFO_selected) {
        ...
        supportOptions.add(_buildSupportOption(
          ...
          doneText:
              'done ${formatDateAndTimeTodayYesterday(_pa.VMMC_INFO_done_date)}',
          onChanged: (bool newState) {
            setState(() {
      ...
      if (sps.YOUNG_MOTHERS_GROUP_selected && _pa.youngMothersAvailable) {
        supportOptions.add(_buildSupportOption(
          ...
          doneText:
              'done ${formatDateAndTimeTodayYesterday(_pa.YOUNG_MOTHERS_GROUP_done_date)}',
          onChanged: (bool newState) {
            setState(() {
      ...
      if (sps.FEMALE_WORTH_GROUP_selected && _pa.femaleWorthAvailable) {
        supportOptions.add(_buildSupportOption(
          ...
          doneText:
              'done ${formatDateAndTimeTodayYesterday(_pa.FEMALE_WORTH_GROUP_done_date)}',
          onChanged: (bool newState) {
            setState(() {
      ...
      if (sps.LEGAL_AID_INFO_selected) {
        supportOptions.add(_buildSupportOption(
          ...
          doneText:
              'done ${formatDateAndTimeTodayYesterday(_pa.LEGAL_AID_INFO_done_date)}',
          onChanged: (bool newState) {
            setState(() {
    ...
    Widget _buildPreferencesCardContent() {
      if (_patient.latestPreferenceAssessment == null) {
        ...
        Widget _vhwInfo() {
          ARTRefillOption lastOption =
              _patient.latestPreferenceAssessment.lastRefillOption;
          ...
          return Column(
            children: [
              _buildRow('VHW Name',
                  _patient.latestPreferenceAssessment.artRefillVHWName),
              _buildRow("VHW's Village",
                  _patient.latestPreferenceAssessment.artRefillVHWVillage),
              _buildRow("VHW's Phone Number",
                  _patient.latestPreferenceAssessment.artRefillVHWPhoneNumber),
            ],
          );
        }
        ...
        Widget _treatmentBuddyInfo() {
          ARTRefillOption lastOption =
              _patient.latestPreferenceAssessment.lastRefillOption;
          if (lastOption == null ||
              lastOption != ARTRefillOption.TREATMENT_BUDDY()) {
            return Container();
          }
          return Column(
            children: [
              _buildRow(
                  "Treatment Buddy's ART Nr.",
                  _patient
                      .latestPreferenceAssessment.artRefillTreatmentBuddyART),
              _buildRow(
                  "Treatment Buddy's Village",
                  _patient.latestPreferenceAssessment
                      .artRefillTreatmentBuddyVillage),
              _buildRow(
                  "Treatment Buddy's Phone Number",
                  _patient.latestPreferenceAssessment
        ...
        Widget _adherenceReminderInfo() {
          bool enabled =
              _patient.latestPreferenceAssessment.adherenceReminderEnabled;
          ...
          return Column(
            children: [
              _buildRow(
                  'Adherence Reminder Frequency',
                  _patient.latestPreferenceAssessment.adherenceReminderFrequency
                      ?.description),
              _buildRow(
                  'Adherence Reminder Notification Time',
                  formatTime(_patient
                      .latestPreferenceAssessment.adherenceReminderTime)),
              _buildRow(
                  'Adherence Reminder Message',
                  _patient.latestPreferenceAssessment.adherenceReminderMessage
                      ?.description),
            ],
          );
        }
        ...
        Widget _refillReminderInfo() {
          bool enabled =
              _patient.latestPreferenceAssessment.artRefillReminderEnabled;
          if (enabled == null || !enabled) {
          ...
          return Column(
            children: [
              _buildRow(
                  'ART Refill Reminder Time',
                  _patient.latestPreferenceAssessment
                      .artRefillReminderDaysBefore?.description),
              _buildRow(
                  'ART Refill Reminder Message',
                  _patient.latestPreferenceAssessment.artRefillReminderMessage
                      ?.description),
            ],
          );
        }
        ...
        Widget _vlNotificationInfo() {
          bool enabled =
              _patient.latestPreferenceAssessment.vlNotificationEnabled;
          if (enabled == null || !enabled) {
          ...
          return Column(
            children: [
              _buildRow(
                  'Viral Load Message (suppressed)',
                  _patient.latestPreferenceAssessment
                      .vlNotificationMessageSuppressed?.description),
              _buildRow(
                  'Viral Load Message (unsuppressed)',
                  _patient.latestPreferenceAssessment
                      .vlNotificationMessageUnsuppressed?.description),
            ],
          );
        }
        ...
        Widget _psychosocialSupportInfo() {
          final YesNoRefused answer = _patient
              .latestPreferenceAssessment.psychosocialShareSomethingAnswer;
          final bool shareSomething =
              answer != null && answer == YesNoRefused.YES();
          return Column(
            children: [
              _buildRow('Did the participant want to share something?',
                  answer.description),
              shareSomething
                  ? _buildRow(
                  ...
                  : Container(),
              _buildRow('How was the participant doing?',
                  _patient.latestPreferenceAssessment.psychosocialHowDoing),
            ],
          );
        }
        ...
        Widget _unsuppressedVlInfo() {
          final YesNoRefused answer = _patient
              .latestPreferenceAssessment.unsuppressedSafeEnvironmentAnswer;
          if (answer == null) {
            return Padding(
              padding: const EdgeInsets.symmetric(vertical: 5.0),
              child: Text(
          ...
          return Column(
            children: [
              _buildRow(
                  'Does the participant have a safe environment to take the medication?',
                  answer.description),
              notSafe
                  ? _buildRow(
                      'Why is the environment not safe?',
                      _patient
                          .latestPreferenceAssessment.unsuppressedWhyNotSafe)
                  : Container(),
            ],
          );
        }
        ...
        return Column(
          children: [
            SizedBox(height: 5.0),
            _buildSubtitle('ART Refill'),
            Divider(),
            _buildRow(
                'ART Refill',
                _patient
                    .latestPreferenceAssessment.lastRefillOption?.description),
            _vhwInfo(),
            _treatmentBuddyInfo(),
            _buildRow(
                'ART Supply Amount',
                _patient
                    .latestPreferenceAssessment.artSupplyAmount?.description),
            SizedBox(height: _spacingBetweenPreferences),
            _buildSubtitle('Notifications'),
            Divider(),
            _notificationsInfo(),
            SizedBox(height: _spacingBetweenPreferences),
            _buildSubtitle('Support'),
            Divider(),
            _buildSupportOptions(),
            SizedBox(height: _spacingBetweenPreferences),
            _buildSubtitle('Psychosocial Support'),
            Divider(),
            _psychosocialSupportInfo(),
            SizedBox(height: _spacingBetweenPreferences),
            _buildSubtitle('Unsuppressed Viral Load'),
            Divider(),
            _unsuppressedVlInfo(),
          ],
        );
    ...
    return _buildCard(
      title: 'Preferences',
      explanationText: lastestPA == null
          ? null
          : 'These are the participant\'s preferences as specified in the last preference assessment from ${formatDateAndTimeTodayYesterday(_patient.latestPreferenceAssessment.createdDate)}.',
      child: Container(
  ...
  Future<void> _fetchFromDatabasePressed(
      BuildContext context, Patient patient) async {
    setState(() {
      _isFetchingViralLoads = true;
    });
    ...
    try {
      ...
      if (newEntries > 0) {
        message =
            '$newEntries new viral load result${newEntries > 1 ? 's' : ''} found.';
      }
      ...
      final bool discrepancyFound =
          await checkForViralLoadDiscrepancies(patient);
      if (discrepancyFound) {}
    } catch (e, s) {
      error = true;
      ...
      switch (e.runtimeType) {
        case VisibleImpactLoginFailedException:
          message =
              'Login to VisibleImpact failed. Contact the development team.';
          break;
  ...
  Future<void> _startAssessmentPressed(
      BuildContext context, Patient patient) async {
  ...
  Future<void> _manageRefillPressed(
      BuildContext context, Patient patient, String nextRefillDate) async {
